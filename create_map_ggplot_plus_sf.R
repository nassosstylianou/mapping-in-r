...
con.votes.constituency <- read_csv('data/general_election_conservative_party_london_results_2017.csv')

sf.constituency <- st_read('shp/Westminster_Parliamentary_Constituencies_December_2015_Generalised_Clipped_Boundaries_in_Great_Britain.shp') %>%
  st_transform(4326)
...
ggplot2::ggplot(data = sf.constituency.join) + 
  ...
  ggtitle("Conservative voting totals, 2017 election (SF)") +
