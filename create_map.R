...
sf.constituency <- sf::st_read('shp/Westminster_Parliamentary_Constituencies_December_2015_Generalised_Clipped_Boundaries_in_Great_Britain.shp') %>% sf::st_transform(4326)
...
ggplot2::ggplot(data = sf.constituency.join) + 
  ...
  geom_sf(aes(fill = `Candidate Votes`), col = "NA") + 
  scale_fill_gradientn(colours = c("#f1eef6", "#a6bddb", "#74a9cf", "#3690c0","#0570b0", "#034e7b"), limits = c(0, 35000)) +
  ggtitle("Conservative voting totals") +
  theme(plot.title = element_text(face="bold", size=12, hjust=0, color="#555555")) 


ggplot(data=sf.constituency.join, aes(x=long, y=lat, group=group)) + # never forget the grouping aestethic!
